...
sys.stdout.write(
    titleStyle + "\n ~~~~~~~~~~~~Jaleb: Chi-square test of independence at multilevel~~~~~~~~~~~~" +
...
def chisq_colum(df, col_indices):
    ...
    for vertebrate_name in df[vertebrate_col].unique():
        ...
        rownames = combined_data[vertebrate_col].tolist()

        if np.any(contingency_table_np == 0):
    ...
    # Reordenar columnas para que la columna del vertebrado sea la primera columna
    results_table = results_table[[vertebrate_col, 'Test', 'p-value', 'Adjusted p-value', 'Chi square', 'df', 'conf_int', 'odds_ratio', 'Signif']]
...
def main():
    ...
    for i in range(num_cols):
        col_index = int(input(f"Type the number which correpond to Categorical or numeric variable {i+1}: ")) - 1
    ...
    try:
        ...
        # Guardar las tablas de contingencia en el archivo Excel
        with pd.ExcelWriter(contingency_file) as writer:
